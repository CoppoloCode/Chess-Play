    ...
    return (
        <section className="flex flex-col h-full justify-center items-center gap-2">
            {currentGamesQuery.isFetched && currentGamesQuery.data!.length > 0  && currentGamesQuery.data?.map((game)=>{
                return <Link className="flex justify-center items-center rounded-md font-medium bg-slate-100 text-black h-10 w-64" href={!game.playerTwoId ? `/game/${game.id}` : `/onlinegame/${game.id}`}>{!game.playerTwoId ?`Resume game with ${difficulty[game.ai!]} AI` : "game against user"}</Link>
            })}
